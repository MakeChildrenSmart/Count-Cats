...
var interval
//specifies what to hide
var hideResponseSpef = ""
//specifies whether to hide new question or not
var setNewQues = ""
//timeout
var timeout
...
//Hides the thumbs up/down and all that jazz
function hideResponse () {
	if (hideResponseSpef == "wrong") {
		//thumbs up
	...
	else if (hideResponseSpef == "right") {
		//thumbs up
...
function clickedAnswer () {
	if (document.getElementById('TheWholeGame').style.visibility == 'visible' && setNewQues != 'switch') {
		if (this.innerHTML == result.length) {
			...
			hideResponseSpef = 'wrong'
			hideResponse()
			//sets to hide both messages and thumbs up and checkbox in 2 seconds
			hideResponseSpef = 'both'
			setNewQues = 'switch'
			timeout = setTimeout(hideResponse,2000)
			
			//makes new question if score if below 10. Otherwise ends the game
			if (score < 10) {
				//Play sounds
				var chooseSound = Math.floor(Math.random() * 4)
				if (chooseSound == 0) {
				...
				else if (chooseSound == 2) {
					document.getElementById("great3").play()
				}
				else if (chooseSound == 3) {
					document.getElementById("great4").play()
				}
				//new question
			}
			else {
...
function setAnswerEvents () {
	...
	for (i=0; i < allCats.length; i++) {
		if (allCats[i].style.visibility == 'visible') {
			allCats[i].addEventListener("mouseover", function() {
				this.style.backgroundColor = 'White'
			})
			allCats[i].addEventListener("mouseout", function() {
